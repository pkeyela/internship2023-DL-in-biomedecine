...
class IntelDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if image is None:
            # DON"T DO THIS!!! JUST TO IDENTIFY AND FIX THE BUG
            print(f'{image_path} is None!')
            image = cv2.imread(self.annot_df.iloc[0, 1])
    ...
    def visualize(self, number_of_img=10, output_width=12, output_height=6):
        ...
        for i in range(number_of_img):
            idx = np.random.randint(0, len(self.annot_df))
            ...
            if self.transform == None:
                ...
                plt.imshow((0.5+0.5*image).permute(1, 2, 0))
        plt.tight_layout()
